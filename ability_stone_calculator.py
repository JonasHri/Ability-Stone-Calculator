...
def full(shape, val = 0):
    if len(shape) == 1:
        return [val] * shape[0]
    else:
    
        return [full(shape[1:]) for _ in range(shape[0])]
...
def score(s_1, s_2, s_3):
    global w_1
    global w_2
    return w_1 * s_1 + w_2 * s_2 - s_3 - (abs(s_1 - s_2))
...
def make_lookup_table(facet_count):
    ...
    expected_facets = full([6,   f, f, f,     f, f, f], -500)
    print("Gerenrating Table")
    for s_1 in dim:
        for s_2 in dim:
            for s_3 in dim:
                for chance in range(6):
                    expected_facets[chance][0][0][0][s_1][s_2][s_3] = score(s_1, s_2, s_3)
                    ...
                    for s_2 in range(facet_count - r_2)[::-1]:
                        for s_3 in range(facet_count - r_3)[::-1]:
                            for chance in range(6):
                                ...
                                if r_1 > 0:
                                    value = max(value, suc_chance * expected_facets[suc_chance_pointer][r_1 - 1][r_2][r_3][s_1 + 1][s_2][s_3] 
                                    + fail_chance * expected_facets[fail_chance_pointer][r_1 - 1][r_2][r_3][s_1][s_2][s_3])
                                
                                if r_2 > 0:
                                    value = max(value, suc_chance * expected_facets[suc_chance_pointer][r_1][r_2 - 1][r_3][s_1][s_2 + 1][s_3] 
                                    + fail_chance * expected_facets[fail_chance_pointer][r_1][r_2 - 1][r_3][s_1][s_2][s_3])
                                
                                if r_3 > 0:
                                    value = max(value, suc_chance * expected_facets[suc_chance_pointer][r_1][r_2][r_3 - 1][s_1][s_2][s_3 + 1] 
                                    + fail_chance * expected_facets[fail_chance_pointer][r_1][r_2][r_3 - 1][s_1][s_2][s_3])
...
def on_press(key):
    ...
    scores = [get_score(arr, 0), get_score(arr, 1), get_score(arr, 2)]
    print("  ", scores)
    print("   Do row: ", argmax(scores) + 1)


w_1 = input("row 1 weight(default=1): ")
if len(w_1) == 0:
    w_1 = 1
else:
    w_1 = float(w_1)

w_2 = input("row 2 weight(default=1): ")
if len(w_2) == 0:
    w_2 = 1
else:
    w_2 = float(w_2)
